...
export const RoomPage = ({ user, socket }) => {
  ...
  const [color, setColor] = useState("red"); // select color
  ...
  const handleUndo = (e) => {
    ...
    // Check if there are elements to undo
    if (elements.length > 0) {
      setElements((prevElements) => {
        const newElements = [...prevElements];
        const lastElement = newElements.pop(); // Remove last element
        
        // Update the removed elements
        setRemovedElements((prevRemoved) => [...prevRemoved, lastElement]);
        return newElements; // Update state
      });
    }
  };
  ...
  const handleRedo = (e) => {
    ...
    // Check if there are removed elements to redo
    if (removedElements.length > 0) {
      // Get the last removed element
      const lastRemovedElement = removedElements[removedElements.length - 1];
      
      // Update both states in a single render cycle
      setElements(prev => [...prev, lastRemovedElement]);
      setRemovedElements(prev => prev.slice(0, -1));
    }
  };
      ...
      <div className="flex h-screen py-2 md:flex-row flex-col justify-center items-center gap-2">
        ...
        <div className="md:w-[20%] md:mt-2 w-[90vw] h-[100%]  md:max-h-[1000px] flex flex-col p-3 mb-10 md:max-w-[400px] md:mb-0 ">
          ...
          <div className=" md:flex-col flex  justify-between md:h-[100%] w-[100%] ">
            ...
            <a id="download" href={"#"} download={"canvas.jpeg"} onClick={download}>
            <div className="flex justify-between md:justify-center items-center">
              <button
